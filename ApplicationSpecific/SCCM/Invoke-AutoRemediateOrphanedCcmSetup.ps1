...
.NOTES
    ...
    Timeout      : ~15 seconds (launcher exits immediately; payload runs via scheduled task up to 300 seconds)
...
$TaskName   = 'DEX_SCCMRemediateCcmSetup'
$BaseDir    = Join-Path $env:ProgramData 'Omnissa\DEX\SCCM'
$PayloadPs1 = Join-Path $BaseDir 'Invoke-AutoRemediateOrphanedCcmSetup.ps1'
$RegPath    = 'HKLM:\Software\AirWatch\Extension\DEXRecords\SCCM\OrphanedCcmSetup'
...
# that runs the step engine with -RunAsPayload. The launcher exits in ~15 seconds
# (the progress-sample wait is inside the payload, not the launcher), well within
# UEM's script timeout. The scheduled task runs the heavy work outside UEM's
# timeout window and writes results to the registry for DEX sensors to read.
if (-not $RunAsPayload) {
    try {
...
try {
    ...
    Set-ItemProperty -Path $RegPath -Name 'CcmSetupKilled'   -Value ([int]$Script:KilledCcmSetup) -Type DWord
} catch { }
